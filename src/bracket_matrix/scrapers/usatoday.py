...
def _score_article_link(label: str, href: str) -> int:
    ...
    if "bracketology" in label_lower:
        score += 12
    if "bracketology" in href_lower:
        score += 8
    if "usatoday.com/story/" in href_lower or href_lower.startswith("/story/"):
        score += 6
    if "/sports/ncaaw/" in href_lower:
        score += 4
    if "women" in label_lower or "women" in href_lower:
        score += 2
    if "/video/" in href_lower or "/picture-gallery/" in href_lower:
        score -= 10

    return score


def _find_latest_bracketology_article_url(hub_html: str, hub_url: str) -> str:
    ...
    for selector in selectors:
        for anchor in soup.select(selector):
            ...
            if "bracketology" not in label.lower() and "bracketology" not in href.lower():
                continue
...
def _looks_like_team_name(team: str) -> bool:
    ...
    if re.search(r"\d", cleaned):
        return False
    if re.search(r"\b(?:region|first four|last four|seed|projected|record|bracketology)\b", cleaned, flags=re.IGNORECASE):
        return False
    return True
...
def parse_usatoday(
    *,
    ...
    latest_article_url = _find_latest_bracketology_article_url(html, source_url)
    ...
    if not pairs:
        raise RuntimeError("USA Today parser returned no seed/team rows")
